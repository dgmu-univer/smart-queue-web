...
export function CreateExcludedSlot({ levelId }: WithLevelId<unknown>) {
  ...
  const handleCreate = (data: CreateExcludedSlotFormProps) => {
    startTransition(async () => {
      ...
      const result = await createExcludedSlot(levelId, payload);
      if (result.success) {
        form.reset();
        setOpen(false);
        router.refresh();
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor="start">Дата</FieldLabel>
                <DatePicker
                  {...field}
                  calendarProps={{
                    id: 'start',
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  ...
                  <Input {...field} id="startTime" type="time" placeholder="16:00" />
                  {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                </Field>
              )}
            />
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  ...
                  <Input {...field} id="endTime" type="time" placeholder="15:00" />
                  {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                </Field>
              )}
            />
